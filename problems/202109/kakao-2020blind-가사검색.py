...
* 트라이를 정방향, 역방향 두개만 만들고 node에다가 지나간 단어의 숫자별로 갯수를 정리함
* 해설을 보니까 단어의 개수별로 트라이를 만들어도 메모리에 문제가 없는거 같다 (1~10000) 만큼 만들어도 괜찮나봄?
* 그렇게 하면 쿼리의 길이에 따라서 다른 트라이에 진입을 시킬 수 있고 거기서 탐색을 진행하면 된다
* ?가 나올때까지 내려간 다음에 거기 children 개수를 리턴하면 되는듯
'''
