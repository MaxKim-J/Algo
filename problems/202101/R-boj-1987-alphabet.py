...
- 탐색을 하는데 있어서 항상 뭐 꼭 그래프순회식으로 할 필요는 없다
- 이 문제는 BFS로 하는게 성능이 월등히 좋았음 => 최적 depth는 일단 BFS로 가자구
- 연산 : 참 이게 백트랙킹 했을 때의 경우의수를 생각해내기가 힘듬
    ...
    - 백트랙킹 방법 : 알파벳은 총 26개이므로 최대 depth는 26 + 그리고 경계 근처의 칸에서는 4가 아니라 3, 2개의 순회 가능
    - 이런 꼴일때는 시간복잡도를 정확하게 측정하기 어렵긴 하다 입력의 유형에 따라 천차만별이라서
    - 여기까지 생각하고 대충 많이 줄겠군 + 어떻게 해야 답에 빨리 도달하겠군 => BFS, DFS, 재귀 백트랙킹 중 편한 방법으로
'''
...
def bfs():
    ...
    # 이런식으로 같은 경우를 한번만 방문 가능(depth가 늘어나면 어짜피 같은 경우는 없으니)
    queue = set()
    ...
    while queue:
        ...
        #! 사실 나올 수 있는 최대 암호 자리는 26, 트리의 최대 뎁쓰도 26 => 알파벳 개수가
        if result == 26:
            return 26
        for i in range(4):
            nx, ny = x + dx[i], y + dy[i]
            # 바로 그냥 암호에 있는지 없는지 판단함. visit을 따로 둘 필요가 없음
